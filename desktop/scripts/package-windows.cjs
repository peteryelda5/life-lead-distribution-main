...
(async()=>{
 ...
 const release=path.join(dist,'Life-Lead-Distribution-Status-Update');fs.rmSync(release,{recursive:true,force:true});fs.mkdirSync(release,{recursive:true});
 ...
 const output=path.join(dist,'Life-Lead-Distribution-Status-Update.zip');fs.rmSync(output,{force:true});
