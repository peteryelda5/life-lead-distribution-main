...
const caller={auth:{getUser:async()=>({data:{user:cp?{id:'admin'}:null}})},from};
...
const res=await handler(new Request('https://example.test',{method:'POST',body:JSON.stringify({email:'test@example.test',password:'longpassword',full_name:'Test',division:'vivid_life',...body})}));return {status:res.status,body:await res.json(),writes};}
